...
> ⚡ **Примечание по производительности:** Списки ресурсов (`/api/tasks`, `/api/users`, `/api/categories`) кэшируются через **Redis**, что многократно ускоряет отдачу данных при повторных запросах. При создании, обновлении или удалении сущностей кэш автоматически инвалидируется.
